...
class FormatorD8_Sequence implements FormatorD8Interface {
  ...
  public static function sta(TranslatorInterface $translator): callable {
    ...
    return function(
      ...
      return self::create($formula, $formulaToAnything, $translator);
    };
  }
  ...
  /**
   * @param \Donquixote\OCUI\Form\D8\FormatorD8Interface $itemFormator
   * @param \Donquixote\OCUI\Translator\TranslatorInterface $translator
   */
  ...
  private function elementAfterBuild(
    ...
    $enabled = false;
    foreach ($conf as $delta => $itemConf) {
      # list($enabled) = $helper->formulaConfGetStatusAndOptions($itemFormula, $itemConf);
      if (!$enabled) {
        unset($conf[$delta]);
      }
    }
