...
abstract class FormatorD8_TabledragBase implements FormatorD8Interface {
   ...
   * Constructor.
   *
   * @param \Donquixote\Ock\Formula\Sequence\Formula_SequenceInterface $sequenceSchema
   * @param \Drupal\ock\Formator\FormatorD8Interface $optionalItemFormator
   * @param \Donquixote\Ock\Translator\TranslatorInterface $translator
   */
  public function __construct(
    private readonly Formula_SequenceInterface $sequenceSchema,
    private readonly FormatorD8Interface $optionalItemFormator,
   ...
   */
  private function elementValue(array $element, $input, array &$form_state): mixed {
    ...
    if (!static::ASSOC) {
      ...
      $form_state['cfrapi-add_more_keys'][$element_name] = $input['.add_more_key'] ?? '';
   ...
   * @return array
   */
  private function elementProcess(array $element, array $conf, string $form_build_id): array {
    ...
    foreach ($value as $delta => $itemValue) {
      ...
      $itemLabel = $this->sequenceSchema->deltaGetItemLabel($delta);
   ...
   * @return array
   */
  public function ajaxCallback(array $form, array &$form_state): array {
   ...
   * @return array
   */
  private function preRenderItems(array $itemsElement): array {
